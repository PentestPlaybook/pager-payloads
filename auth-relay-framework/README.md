...
Detailed setup instructions are provided in the `Setup/` directory. Follow them in order.

### 1. Pineapple setup (`pager_setup.txt` or `markvii_setup.txt`)

Use the file that matches your device — you only need one.

- Configure PineAP filters, Wireless Client Mode, Management AP, and Evil WPA AP via the Pineapple UI
- Reboot the Pineapple
- Download and run the payloads via SSH to install Evil Portal, configure the captive portal, and set the Evil Portal interface

### 2. Termux setup (`termux_setup.txt`)

- Install required packages in Termux: `openssh`, `geckodriver`, `sshpass`, `selenium`, `termux-x11-nightly`
- Forward ADB port and connect via SSH from PowerShell
- Run the root permission fix commands to set correct SELinux contexts and file permissions
- Optionally back up your Termux environment via ADB
